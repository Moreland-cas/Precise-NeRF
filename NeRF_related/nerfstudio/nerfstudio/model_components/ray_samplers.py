...
class PDFSampler(Sampler):
    ...
    def generate_ray_samples(
        ...
        # TODO: 解决返回frustums数量是193而非192的问题
        weights = weights[..., 0] 
        if self.use_smoothing:
            # 对weights进行最大值和平均值滤波器处理，类似于mip-NeRF的实现
            weights = F.pad(weights, (1, 1), mode='replicate')  # 扩展权重, bs 130
            weights_max = torch.maximum(weights[..., :-1], weights[..., 1:])  # 最大值滤波器, bs 129
            weights = 0.5 * (weights_max[..., :-1] + weights_max[..., 1:])  # 平均值滤波器, bs 128
            
        weights = weights + self.histogram_padding

        # Add small offset to rays with zero weight to prevent NaNs
        weights_sum = torch.sum(weights, dim=-1, keepdim=True) # 4096, 1
...
class VolumetricSampler(Sampler):
    ...
    def get_sigma_fn(self, origins, directions, times=None) -> Optional[Callable]:
        ...
        def sigma_fn(t_starts, t_ends, ray_indices):
            # TODO：这里也需要改一下，重新给对于线段的density_fn提供输入
