...
class MipNerfModel(Model):
    ...
    def populate_modules(self):
        ...
        position_encoding = NeRFEncoding(
            # in_dim=3, num_frequencies=16, min_freq_exp=0.0, max_freq_exp=15.0, include_input=False # 原论文
            in_dim=3, num_frequencies=16, min_freq_exp=0.0, max_freq_exp=15.0, include_input=True # nefstudio
        )
        direction_encoding = NeRFEncoding(
            in_dim=3, num_frequencies=4, min_freq_exp=0.0, max_freq_exp=3.0, include_input=True # nefstudio
        )

        self.field = NeRFField(
            position_encoding=position_encoding, direction_encoding=direction_encoding, use_integrated_encoding=True # original
        )

        # samplers
        self.sampler_uniform = UniformSampler(num_samples=self.config.num_coarse_samples)
        # self.sampler_pdf = PDFSampler(num_samples=self.config.num_importance_samples, include_original=False, use_smoothing=True) # mipnerf paper
        self.sampler_pdf = PDFSampler(num_samples=self.config.num_importance_samples, include_original=False, use_smoothing=False) # nerfstudio
