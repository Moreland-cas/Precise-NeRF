...
class Frustums(TensorDataclass):
    ...
    def get_start_end_positions(self) -> Float[Tensor, "*batch 6"]:
        """Calculates "start" position of frustum.

        Returns:
            xyz positions.
        """
        start = self.origins + self.directions * self.starts
        end = self.origins + self.directions * self.ends
...
class RaySamples(TensorDataclass):
    ...
    def get_weights(self, densities: Float[Tensor, "*batch num_samples 1"], version) -> Float[Tensor, "*batch num_samples 1"]:
        ...
        transmittance = torch.cumsum(
            delta_density[..., :-1, :], # batch num_samples-1 1
        dim=-2)
        ...
        transmittance = torch.exp(-transmittance)  # [..., "num_samples"] 这里是累计的透光率

        if version == 'v1':
            alphas = 1 - torch.exp(-delta_density) # 1 - exp(-sigma * dt)
        elif version == 'v2':
            alphas = delta_density # sigma * dt
    ...
    def merge_frustums(self) -> Tuple['RaySamples', Tensor]:
        # 改这个为不重叠的合并输入数据的相邻两个段，因此产生的新段数量自然为1/2
        bs, num_frustums = self.frustums.shape # 4096, 64
        
        # 选择起始索引 （end也被包含在里面）
        n1 = num_frustums//2
        n2 = 2
        start_indices = 2 * torch.arange(0, num_frustums//2)
        start_indices = start_indices.view(1, -1, 1).repeat(bs, 1, 1).long().to(self.frustums.starts.device) # bs, n1, 1
        end_indices = start_indices + 1
        # 批量获取 starts 和 ends // spacing_starts: 4096, 64, 1
        new_ray_starts = torch.gather(self.spacing_starts, 1, start_indices) # 4096, 10, 1
        new_ray_ends = torch.gather(self.spacing_ends, 1, end_indices)

        # 批量获取 Frustum starts 和 ends // frustums.starts: 4096, 64, 1
        new_frustum_starts = torch.gather(self.frustums.starts, 1, start_indices) # 4096, 10, 1
        new_frustum_ends = torch.gather(self.frustums.ends, 1, end_indices)

        # 更新 camera_indices
        if self.camera_indices is not None:
            new_camera_indices = torch.gather(self.camera_indices, 1, start_indices) # 4096, 10, 1
        
        # 初始化 new_deltas // self.deltas: 4096, 64, 1
        new_deltas = torch.zeros(bs, n1, 1, device=self.deltas.device)

        # 扩展 start_indices 以包含所有需要合并的 frustums 的索引
        expanded_indices = start_indices[..., None, :] + torch.arange(n2, device=start_indices.device).view(1, 1, n2, 1) # 4096, n1, n2, 1
        expanded_indices = einops.rearrange(expanded_indices, 'bs n1 n2 1 -> bs (n1 n2) 1')

        # 使用扩展后的索引来批量提取 deltas
        extracted_deltas = torch.gather(self.deltas, 1, expanded_indices) # bs (n1 n2) 1
        extracted_deltas = einops.rearrange(extracted_deltas, 'bs (n1 n2) 1 -> bs n1 n2', n1=n1, n2=n2)

        # 对提取的 deltas 求和以获得每个新大 frustum 的 new_deltas
        new_deltas = extracted_deltas.sum(dim=-1, keepdim=True) # bs, n1, 1
        
        # 创建新的 Frustums 实例
        new_frustums = Frustums(
            origins=self.frustums.origins[:, 0:1, :].expand(-1, n1, -1),
            directions=self.frustums.directions[:, 0:1, :].expand(-1, n1, -1),
            starts=new_frustum_starts,
            ends=new_frustum_ends,
            pixel_area=self.frustums.pixel_area[:, 0:1, :].expand(-1, n1, -1)
        )
        
        new_ray_samples = RaySamples(
            frustums=new_frustums,
            camera_indices=new_camera_indices if self.camera_indices is not None else None,
            deltas=new_deltas,
            spacing_starts=new_ray_starts,
            spacing_ends=new_ray_ends
        )
        # 这里似乎不用产生新的Raysample，只需要新的frustum就行
        new_indexes = torch.cat([start_indices, end_indices], dim=-1) # bs, n1, 2
        # 返回新的 RaySamples 实例
        # return new_ray_samples, start_indices
        return new_ray_samples
