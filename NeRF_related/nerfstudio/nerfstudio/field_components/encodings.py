...
class HashEncoding(Encoding):
    ...
    def __init__(
        ...
        self.hash_offset = levels * self.hash_table_size # 代表不同粒度hash表的起始index，它实际是把不同hash表合并存到一个大表里的
    ...
    def build_nn_modules(self) -> None:
        ...
        self.hash_table *= self.hash_init_scale # TODO：这里对于线段的分解方法，可能要加一个初始尺度，以应对短线段分解到长边上
    ...
    def hash_fn(self, in_tensor: Int[Tensor, "*bs num_levels 3"]) -> Shaped[Tensor, "*bs num_levels"]:
        ...
        """
        # TODO: 在这里改变hash函数，改为输入一个线段的表示，输出该线段的index，可以考虑把每个线段的特征存在他的中点处
