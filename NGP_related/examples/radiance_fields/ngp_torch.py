...
class NGPRadianceField(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.geo_feat_dim > 0:
            self.mlp_head = tcnn.Network(
                n_input_dims=(
                    (
                        self.direction_encoding.n_output_dims # 16
                        if self.use_viewdirs
                        else 0
                    )
    ...
    def query_density(self, x, return_feat: bool = False):
        ...
        x = (
            self.mlp_base(x.view(-1, self.num_dim)) # torch.Size([2097152, 16])
    ...
    def _query_rgb(self, dir, embedding, apply_act: bool = True):
        ...
        if self.use_viewdirs:
            dir = (dir + 1.0) / 2.0
            d = self.direction_encoding(dir.reshape(-1, dir.shape[-1])) # torch.Size([285983, 16])
            h = torch.cat([d, embedding.reshape(-1, self.geo_feat_dim)], dim=-1) # torch.Size([285983, 31])
        else:
...
class MLPWithHashEncoding(nn.Module):
    ...
    def build_nn_modules(self) -> None:
        ...
        # self.mlp = MLP(
        #     in_dim=self.hash_encoder.get_out_dim(),
        ...
        self.mlp = tcnn.Network(
                n_input_dims=self.num_levels*self.features_per_level,
    ...
    def forward(self, in_tensor):
        # Float[Tensor, "*bs in_dim"]) -> Float[Tensor, "*bs out_dim"]
        hash_result = self.hash_encoder(in_tensor)
        out_tensor = self.mlp(hash_result)
        return out_tensor
