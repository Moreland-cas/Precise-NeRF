...
class SubjectLoader(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.images = torch.from_numpy(self.images).to(torch.uint8) # (100, 800, 800, 4)
        self.camtoworlds = torch.from_numpy(self.camtoworlds).to(torch.float32) # 100, 4, 4
        self.K = torch.tensor(
            [
        ...
        self.images = self.images.to(device) # (100, 800, 800, 4)
        self.camtoworlds = self.camtoworlds.to(device) # 100, 4, 4
        self.K = self.K.to(device) # 3， 3
