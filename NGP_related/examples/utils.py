...
def render_image_with_occgrid(
    ...
    for i in range(0, num_rays, chunk):
        ...
        rays_o = chunk_rays.origins # 1024 3
        rays_d = chunk_rays.viewdirs # 1024 3

        def sigma_fn(t_starts, t_ends, ray_indices):
            t_origins = rays_o[ray_indices] # B 3
...
def render_image_with_segoccgrid(
    # scene
    radiance_field: torch.nn.Module,
    estimator: OccGridEstimator,
    rays: Rays,
    ...
    for i in range(0, num_rays, chunk):
        ...
        def sigma_fn(t_starts, t_ends, ray_indices):
            t_origins = rays_o[ray_indices]
            t_dirs = rays_d[ray_indices] # B 3
            
            seg_start = t_origins + t_dirs * t_starts[:, None]
            seg_end = t_origins + t_dirs * t_ends[:, None]
            seg_positions = torch.cat([seg_start, seg_end], dim=-1)
            sigmas = radiance_field.query_density(seg_positions) # B 1
            return sigmas.squeeze(-1)
        ...
        def rgb_sigma_fn(t_starts, t_ends, ray_indices):
            ...
            seg_end = t_origins + t_dirs * t_ends[:, None]
            seg_positions = torch.cat([seg_start, seg_end], dim=-1)
            rgbs, sigmas = radiance_field(seg_positions, t_dirs)
            return rgbs, sigmas.squeeze(-1)
