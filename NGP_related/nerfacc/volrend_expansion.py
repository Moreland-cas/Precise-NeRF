...
from .scan import exclusive_prod, exclusive_sum, inclusive_sum


def rendering_expansion(
    ...
    # radiance field
    rgb_sigma_fn: Optional[Callable] = None, # 用的是这个
    ...
    if rgb_sigma_fn is not None:
        if t_starts.shape[0] != 0:
            # 通过rgb_sigma_fn得到rgbs和sigmas, b 9; b 3
            rgbs, sigmas = rgb_sigma_fn(t_starts, t_ends, ray_indices)
        else:
            # 对于没有采样点的情况
            rgbs = torch.empty((0, 9), device=t_starts.device)
            sigmas = torch.empty((0, 3), device=t_starts.device)
        
        # Rendering: compute weights.
        
        seg_trans, weight_cbg = render_segment_Transmittance_from_density_expansion( # B
            t_starts,
            t_ends,
            sigmas,
            ray_indices=ray_indices,
            n_rays=n_rays,
        ) # 1 T1 T1*T2 ... T1***Tn-1; n_rays
        # extras = {
        #     "weights": weights,
        #     "alphas": alphas,
        #     "trans": seg_trans, # B
        ...
        #     "rgbs": rgbs, # B 9
        # }
    elif rgb_alpha_fn is not None and False:
        if t_starts.shape[0] != 0:
    ...
    # 由 rgbs 计算 C_t1_t2
    def calculate_C_from_rgbs(sigmas, rgbs, delta):
        # rgbs: B 9 
        # sigmas: B 3 
        # delta:B 1
        delta = delta[..., None] # B 1 1
        s0, s1, s2 = torch.chunk(sigmas[..., None], chunks=3, dim=-2) # B 1 1
        c0, c1, c2 = torch.chunk(rgbs[..., None], chunks=3, dim=-2) # B 3 1
        # C_t1_t2 = delta*c0*s0  - delta**2 * c0*s0**2/2
        # C_t1_t2 = (1-torch.exp(-s0 * delta)) * c0
        C_t1_t2 = delta*c0*s0 + delta**2*(-c0*s0**2/2 + c0*s1/2 + c1*s0/2) + delta**3*(c0*s0**3/6 - c0*s0*s1/2 + c0*s2/6 - c1*s0**2/3 + c1*s1/3 + c2*s0/6)
        C_t1_t2 = C_t1_t2.squeeze(dim=-1) # B 3 1 -> B 3
        return C_t1_t2
    C_t1_t2 = calculate_C_from_rgbs(sigmas, rgbs, (t_ends - t_starts)[..., None]) # B 3
    colors = accumulate_along_rays(
        seg_trans, values=C_t1_t2, ray_indices=ray_indices, n_rays=n_rays
    ) # n_rays 3
    ...
    if render_bkgd is not None:
        # colors = colors + render_bkgd * (1.0 - opacities) # 原本的
        colors = colors + render_bkgd * weight_cbg # 先版本把 opacities 换为 seg_trans 的最后一项，即 T1***Tn
...
def render_transmittance_from_density_expansion(
    t_starts: Tensor,
    t_ends: Tensor,
    sigmas: Tensor, # B 3, sigma(x)的多阶导数
    packed_info: Optional[Tensor] = None, # None
    ray_indices: Optional[Tensor] = None, # B
    n_rays: Optional[int] = None, # 1024
    prefix_trans: Optional[Tensor] = None,
) -> Tuple[Tensor, Tensor]:
    ...
    if not is_cub_available() and packed_info is None:
        ...
        packed_info = pack_info(ray_indices, n_rays)
        ray_indices = None
     # 计算每一段的 transmittance
    s0, s1, s2 = torch.chunk(sigmas, chunks=3, dim=-1) # each: B 1
    delta = (t_ends - t_starts)[:, None] # B 1
    
    # 原本的 sigmas_dt 是 delta*s0
    # TODO: 改为以下实现方式
    # sigmas_dt = 1. - delta*s0 + delta**2*(s0**2/2 - s1/2) + delta**3*(-s0**3/6 + s0*s1/2 - s2/6) # b 1
    # sigmas_dt = sigmas_dt.squeeze(dim=-1) # B
    # 现在的实现方式是把T(t1, t2)仅在次幂上展开
    # delta**3*s2/6 + delta**2*s1/2 + delta*s0
    # sigmas_dt = delta*s0
    sigmas_dt = delta**3*s2/6. + delta**2*s1/2. + delta*s0 # B 1
    sigmas_dt_sq = sigmas_dt.squeeze(dim=-1) # B

    trans = torch.exp(
        -exclusive_sum(sigmas_dt_sq, packed_info=packed_info, indices=ray_indices)
    ) # 1 T1 ... T1***Tn-1
    # 额外返回一个对于每条光线的 T1***Tn，大小为 n_rays
    weight_cbg = torch.zeros( # n_rays
        (n_rays, 1), device=sigmas_dt.device, dtype=sigmas_dt.dtype
    )
    weight_cbg.index_add_(0, ray_indices, sigmas_dt) # n_rays
    weight_cbg = torch.exp(-weight_cbg) # n_rays
    if prefix_trans is not None:
        assert "should not be here"
        import pdb;pdb.set_trace()
        trans = trans * prefix_trans
    return trans, weight_cbg
...
# def render_weight_from_density_expansion(
def render_segment_Transmittance_from_density_expansion(
    t_starts: Tensor, # B
    t_ends: Tensor, # B
    sigmas: Tensor, # B
    packed_info: Optional[Tensor] = None,   # B 2
    ray_indices: Optional[Tensor] = None,   # B
    n_rays: Optional[int] = None,           # 1024
    prefix_trans: Optional[Tensor] = None,
    ...
    返回B段每一段的T
    """
    # 这里原本的 sigmas 输入大小为 B，现在为 B 3
    # 对于全新版本，只需要计算 transmittance， 这里的 transmittance 是从0到当前位置
    trans, weight_cbg = render_transmittance_from_density_expansion(
