...
class PyNeRFOccupancyModel(PyNeRFBaseModel):
    ...
    def get_image_metrics_and_images(
        ...
        global counter
        scene = "ship"
        res = (list(images_dict.keys())[-3]).split("_")[-1]
        save_path = f"/data/zhangboyuan/Programs/pynerf_dev/renders/vanilla/{scene}/{res}/"
        import os
        os.makedirs(save_path, exist_ok=True)
        save_name = f"{str(counter)}.png"
        from torchvision.utils import save_image
        tensor_image = images_dict["img"].permute(2, 0, 1)
        save_image(tensor_image, save_path + save_name)
        counter += 1
