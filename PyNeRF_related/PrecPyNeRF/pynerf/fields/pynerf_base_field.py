...
class PyNeRFBaseField(Field):
    ...
    def __init__(
        ...
        self.share_feature_grid = share_feature_grid # True
        ...
        self.trained_level_resolution = trained_level_resolution # 128
        ...
        self.encoding_input_dims = encoding_input_dims # [64, 64, ...] 8
        ...
        for encoding_input_dim in encoding_input_dims:
            mlp_bases.append(MLP(
                in_dim=encoding_input_dim, # 64 * 2
                out_dim=1 + self.geo_feat_dim,
                num_layers=num_layers, # 2
                layer_width=hidden_dim, # 64
                activation=nn.ReLU(),
                out_activation=None,
                implementation="tcnn"
            ))
        ...
        if output_interpolation == OutputInterpolation.COLOR: # TRUE
            mlp_heads = []
    ...
    def get_density(self, ray_samples: RaySamples, update_levels: bool = True):
        if True and "start":
            positions = ray_samples.frustums.origins + ray_samples.frustums.directions * ray_samples.frustums.starts
            if self.spatial_distortion is not None:
                ...
                positions_flat = positions.view(-1, 3)
            positions_flat_start = positions_flat
        if True and "end":
            positions = ray_samples.frustums.origins + ray_samples.frustums.directions * ray_samples.frustums.ends
            if self.spatial_distortion is not None:
                positions = self.spatial_distortion(positions)
                positions = (positions + 2.0) / 4.0
                positions_flat = positions.view(-1, 3)
                ...
                positions = SceneBox.get_normalized_positions(positions, self.aabb)
                positions_flat = positions.view(-1, 3)
            positions_flat_end = positions_flat
            
        positions_flat_mid = (positions_flat_end + positions_flat_start) / 2.
        ...
        if explicit_level:
            ...
            pixel_levels = self.base_log - torch.log(pixel_widths) / self.log_scale_factor # B [6.3895, 3.9810, 4.9644,  ..., 4.0103, 3.4844, 4.9837]
            if self.trained_level_resolution is not None: # 128
                reso_indices = (positions_flat_mid * self.trained_level_resolution).floor().long().clamp(0,
        ...
        if self.share_feature_grid: 
            encoding_start = self.get_shared_encoding(positions_flat_start) # B, 64(16 * 4)
            encoding_end = self.get_shared_encoding(positions_flat_end)
            encoding = torch.cat([encoding_start, encoding_end], dim=-1) # B, 128
        ...
        for level, cur_level_indices in level_indices.items():
            if self.share_feature_grid:
                level_encoding = encoding[cur_level_indices][..., :self.encoding_input_dims[level]] # self.encoding_input_dims 
            else: 
