...
class PyNeRFField(PyNeRFBaseField):
    ...
    def __init__(
        ...
        super().__init__(aabb, num_images, [2 * num_levels * features_per_level for _ in range(num_scales)], num_layers,
        ...
        if not share_feature_grid:
            ...
            per_level_scale = math.exp(math.log(max_resolution / base_resolution) / (num_levels - 1)) # max_res=1024

            self.encoding = tcnn.Encoding(
                n_input_dims=3,
                encoding_config={
                    'otype': 'HashGrid',
                    'n_levels': num_levels, # 16
                    'n_features_per_level': features_per_level, # 4
                    'log2_hashmap_size': log2_hashmap_size,
                    'base_resolution': base_resolution,
